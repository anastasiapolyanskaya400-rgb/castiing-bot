...
(
    LANGUAGE, NAME, AGE, CITY, HEIGHT, WEIGHT,
    MEASUREMENTS, SHOE_SIZE, HAIR_COLOR, EYE_COLOR,
    EXPERIENCE, ABROAD, BOOK, PHOTO,
) = range(14)

STRINGS = {
    "ru": {
        "welcome": ("👋 Привет! Добро пожаловать в кастинг-бот для моделей.\n\nЯ задам тебе несколько вопросов, а затем попрошу фото.\nДля отмены в любой момент напиши /cancel.\n\n📝 Давай начнём!\n\nВведи своё *имя и фамилию*:"),
        ...
        "ask_weight": "⚖️ Твой *вес* (в кг, например: 55)?",
        ...
        "send_error": "⚠️ Заявка принята, но возникла ошибка при отправке администратору. Пожалуйста, свяжитесь с нами напрямую.",
        "caption_header": "📋 *НОВАЯ ЗАЯВКА НА КАСТИНГ*", "caption_lang": "🌐 *Язык:* Русский",
        "caption_name": "👤 *Имя:*", "caption_age": "🎂 *Возраст:*", "caption_city": "🏙 *Город:*",
        "caption_height": "📏 *Рост:*", "caption_height_unit": "см",
        "caption_weight": "⚖️ *Вес:*", "caption_weight_unit": "кг",
        "caption_measurements": "📐 *Параметры (г/т/б):*", "caption_shoe": "👟 *Размер обуви:*",
        "caption_hair": "💇 *Цвет волос:*", "caption_eyes": "👁 *Цвет глаз:*",
        "caption_experience": "🌟 *Опыт:*", "caption_abroad": "✈️ *Зарубежные контракты:*",
        "caption_tg": "📱 *Telegram:*",
    },
    "en": {
        "welcome": ("👋 Hi! Welcome to the model casting bot.\n\nI'll ask you a few questions and then request a photo.\nTo cancel at any time, type /cancel.\n\n📝 Let's get started!\n\nEnter your *full name*:"),
        ...
        "ask_weight": "⚖️ Your *weight* (in kg, e.g. 55)?",
        ...
        "send_error": "⚠️ Application received, but there was an error sending it to the admin. Please contact us directly.",
        "caption_header": "📋 *NEW CASTING APPLICATION*", "caption_lang": "🌐 *Language:* English",
        "caption_name": "👤 *Name:*", "caption_age": "🎂 *Age:*", "caption_city": "🏙 *City:*",
        "caption_height": "📏 *Height:*", "caption_height_unit": "cm",
        "caption_weight": "⚖️ *Weight:*", "caption_weight_unit": "kg",
        "caption_measurements": "📐 *Measurements (B/W/H):*", "caption_shoe": "👟 *Shoe size:*",
        "caption_hair": "💇 *Hair colour:*", "caption_eyes": "👁 *Eye colour:*",
        "caption_experience": "🌟 *Experience:*", "caption_abroad": "✈️ *International contracts:*",
        "caption_tg": "📱 *Telegram:*",
    },
}
...
async def start(update, context):
    ...
    await update.message.reply_text("🌐 Выбери язык / Choose your language:",
        reply_markup=ReplyKeyboardMarkup(keyboard, one_time_keyboard=True, resize_keyboard=True))
    return LANGUAGE

async def language(update, context):
    text = update.message.text.strip()
    context.user_data["lang"] = "en" if ("English" in text or "english" in text.lower()) else "ru"
    lang = lang_of(context)
    await update.message.reply_text(t("welcome", lang), parse_mode="Markdown", reply_markup=ReplyKeyboardRemove())
    return NAME

async def name(update, context):
    context.user_data["name"] = update.message.text.strip()
    await update.message.reply_text(t("ask_age", lang_of(context)), parse_mode="Markdown")
    return AGE

async def age(update, context):
    text = update.message.text.strip()
    if not text.isdigit() or not (14 <= int(text) <= 80):
        await update.message.reply_text(t("err_age", lang_of(context)))
        return AGE
    context.user_data["age"] = text
    await update.message.reply_text(t("ask_city", lang_of(context)), parse_mode="Markdown")
    return CITY

async def city(update, context):
    context.user_data["city"] = update.message.text.strip()
    await update.message.reply_text(t("ask_height", lang_of(context)), parse_mode="Markdown")
    return HEIGHT

async def height(update, context):
    text = update.message.text.strip()
    if not text.isdigit() or not (140 <= int(text) <= 220):
        await update.message.reply_text(t("err_height", lang_of(context)))
        return HEIGHT
    context.user_data["height"] = text
    await update.message.reply_text(t("ask_weight", lang_of(context)), parse_mode="Markdown")
    return WEIGHT
...
async def weight(update, context):
    ...
    try:
        ...
        await update.message.reply_text(t("err_weight", lang_of(context)))
        return WEIGHT
    context.user_data["weight"] = text
    await update.message.reply_text(t("ask_measurements", lang_of(context)), parse_mode="Markdown")
    return MEASUREMENTS
...
async def measurements(update, context):
    ...
    if len(parts) != 3 or not all(p.isdigit() for p in parts):
        await update.message.reply_text(t("err_measurements", lang_of(context)))
        return MEASUREMENTS
    context.user_data["measurements"] = text
    await update.message.reply_text(t("ask_shoe", lang_of(context)), parse_mode="Markdown")
    return SHOE_SIZE

async def shoe_size(update, context):
    text = update.message.text.strip()
    if not text.replace(".", "").isdigit():
        await update.message.reply_text(t("err_shoe", lang_of(context)))
        return SHOE_SIZE
    context.user_data["shoe_size"] = text
    await update.message.reply_text(t("ask_hair", lang_of(context)), parse_mode="Markdown")
    return HAIR_COLOR

async def hair_color(update, context):
    context.user_data["hair_color"] = update.message.text.strip()
    await update.message.reply_text(t("ask_eyes", lang_of(context)), parse_mode="Markdown")
    return EYE_COLOR

async def eye_color(update, context):
    context.user_data["eye_color"] = update.message.text.strip()
    await update.message.reply_text(t("ask_experience", lang_of(context)), parse_mode="Markdown")
    return EXPERIENCE
...
async def experience(update, context):
    ...
    await update.message.reply_text(t("ask_abroad", lang), parse_mode="Markdown",
        reply_markup=ReplyKeyboardMarkup(keyboard, one_time_keyboard=True, resize_keyboard=True))
    return ABROAD

async def abroad(update, context):
    lang = lang_of(context)
    text = update.message.text.strip()
    ...
    if text not in yes_vals and text not in no_vals:
        keyboard = [[t("abroad_yes", lang), t("abroad_no", lang)]]
        await update.message.reply_text(t("err_abroad", lang),
            reply_markup=ReplyKeyboardMarkup(keyboard, one_time_keyboard=True, resize_keyboard=True))
        return ABROAD
    context.user_data["abroad"] = t("abroad_yes_val", lang) if text in yes_vals else t("abroad_no_val", lang)
    await update.message.reply_text(t("ask_book", lang), parse_mode="Markdown", reply_markup=ReplyKeyboardRemove())
    return BOOK

async def book(update, context):
    text = update.message.text.strip()
    if not text:
        await update.message.reply_text(t("err_book", lang_of(context)))
        return BOOK
    context.user_data["book"] = text
    await update.message.reply_text(t("ask_photo", lang_of(context)), parse_mode="Markdown")
    return PHOTO
...
async def photo(update, context):
    ...
    if update.message.text:
        text = update.message.text.strip()
        if text in (done_btn, "✅ Done", "✅ Готово", "Done", "Готово", "done", "готово"):
            if not photos:
    ...
    if not update.message.photo:
        ...
        return PHOTO
    if len(photos) >= 5:
        await update.message.reply_text(t("photo_max", lang),
            reply_markup=ReplyKeyboardMarkup([[done_btn]], one_time_keyboard=True, resize_keyboard=True))
        return PHOTO
    photos.append(update.message.photo[-1].file_id)
    count = len(photos)
    if count < 5:
        await update.message.reply_text(t("photo_received", lang).format(count), parse_mode="Markdown",
            reply_markup=ReplyKeyboardMarkup([[more_btn, done_btn]], one_time_keyboard=True, resize_keyboard=True))
        return PHOTO
    return await _submit_application(update, context, lang, user, data, photos)
...
async def _submit_application(update, context, lang, user, data, photos):
    ...
    try:
        if len(photos) == 1:
            await context.bot.send_photo(chat_id=ADMIN_CHAT_ID, photo=photos[0], caption=caption, parse_mode="Markdown")
        else:
    ...
    await update.message.reply_text(t("success", lang), parse_mode="Markdown", reply_markup=ReplyKeyboardRemove())
    context.user_data.clear()
    return ConversationHandler.END

async def cancel(update, context):
    await update.message.reply_text(t("cancel", lang_of(context)), reply_markup=ReplyKeyboardRemove())
    context.user_data.clear()
    return ConversationHandler.END

async def test_admin(update, context):
    if update.effective_user.id != ADMIN_CHAT_ID: return
    try:
        await context.bot.send_message(chat_id=ADMIN_CHAT_ID,
            text=f"✅ Test OK! ADMIN_CHAT_ID = {ADMIN_CHAT_ID}")
...
def main():
    ...
    logger.info("Bot started. Polling (timeout=30s)...")
